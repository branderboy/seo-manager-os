      ...
      <CardContent>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-slate-400">Rankings · Traffic · Leads · Calls · AI Visibility · Revenue</div>
          </div>
        </div>
          ...
          <div className="mt-2 pl-8">
            ...
            {status === "pending" && (
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm text-slate-500">Sent to {manager} · awaiting sign-off</span>
                <button
...
function StepLabel({ n, done, locked, children }: { n: number; done?: boolean; locked?: boolean; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className={cn("text-sm font-semibold", locked ? "text-slate-400" : "text-slate-800")}>{children}</span>
    </div>
  );
}
...
function ShareTarget({
  ...
  return (
    ...
    >
      <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", disabled ? "bg-slate-100 text-slate-400" : "bg-accent-50 text-accent-600")}>
        {icon}
      </span>
