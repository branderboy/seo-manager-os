    ...
    <div className="rounded-2xl border border-slate-200/70 bg-white p-7 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            ...
            <span className="font-mono text-slate-500">{current.at}</span>
          </p>
        </div>
        {!adding && (
          <button
      ...
      {adding && (
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <label className="mb-2 block text-[13px] font-bold uppercase tracking-wide text-slate-500">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-100"
            >
          ...
          return (
            <li key={v.version} className={cn("flex items-center justify-between gap-3 px-4 py-3", isCurrent && "bg-emerald-50/40")}>
              <div className="flex min-w-0 items-center gap-3">
                <span className={cn("inline-flex shrink-0 items-center rounded-md px-2 py-0.5 text-xs font-bold", isCurrent ? "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200" : "bg-slate-100 text-slate-600")}>
                  {v.version}
                </span>
                <div className="min-w-0">
                  <div className="truncate text-sm text-slate-700">{v.note}</div>
                  <div className="text-xs text-slate-400">{v.author}</div>
                </div>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <span className="hidden font-mono text-xs text-slate-500 sm:inline">{v.at}</span>
                {isCurrent ? (
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600 transition-colors hover:border-[#1DA1F2] hover:text-[#1DA1F2]"
                  >
