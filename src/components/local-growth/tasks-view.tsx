...
function Board({ rows, markDone, copyUpdate, copied }: { rows: SeoTask[]; markDone: (id: string) => void; copyUpdate: (task: SeoTask) => void; copied: string | null }) {
  return <div className="grid gap-3 2xl:grid-cols-6">{boardColumns.map((column) => <section key={column.status} className="rounded-xl bg-slate-100/70 p-2.5"><div className="mb-2 flex items-center justify-between px-1"><span className="text-xs font-semibold text-slate-700">{column.label}</span><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-slate-500">{rows.filter((row) => row.status === column.status).length}</span></div><div className="space-y-2.5">{rows.filter((row) => row.status === column.status).map((task) => <TaskCard key={task.id} task={task} markDone={markDone} copyUpdate={copyUpdate} copied={copied} />)}{rows.filter((row) => row.status === column.status).length === 0 && <div className="rounded-lg border border-dashed border-slate-300 p-4 text-center text-[11px] text-slate-400">Empty</div>}</div></section>)}</div>;
}
...
function TaskCard({ task, markDone, copyUpdate, copied }: { task: SeoTask; markDone: (id: string) => void; copyUpdate: (task: SeoTask) => void; copied: string | null }) { const campaign = demoCampaigns.find((item) => item.id === task.campaignId); return <article className="rounded-lg border border-[var(--border)] bg-white p-3 shadow-[var(--shadow-card)]"><div className="flex items-start justify-between gap-2"><Priority value={task.priority} /><span className="text-[10px] text-slate-400">{task.estimatedMinutes}m</span></div><h3 className="mt-2 text-xs font-semibold leading-5 text-slate-900">{task.title}</h3><div className="mt-1 text-[10px] leading-4 text-slate-500">{campaign?.clientName}</div>{task.dependency && <div className="mt-2 rounded-md bg-amber-50 p-2 text-[10px] leading-4 text-amber-800">{task.dependency}</div>}<div className="mt-3 flex items-center justify-between text-[10px] text-slate-500"><span>{task.owner.split(" ")[0]}</span><span className="inline-flex items-center gap-1"><Clock3 className="h-3 w-3" />{task.dueDate.slice(5)}</span></div><div className="mt-2 flex gap-1.5 border-t border-[var(--border)] pt-2"><button onClick={() => copyUpdate(task)} className="rounded-md p-1.5 text-slate-400 hover:bg-slate-50" title="Copy Slack-ready status"><Copy className="h-3.5 w-3.5" /></button>{task.status !== "completed" && <button onClick={() => markDone(task.id)} className="rounded-md p-1.5 text-emerald-600 hover:bg-emerald-50" title="Mark complete"><CheckCircle2 className="h-3.5 w-3.5" /></button>}{copied === task.id && <span className="self-center text-[9px] font-semibold text-emerald-700">Copied</span>}</div></article>; }
