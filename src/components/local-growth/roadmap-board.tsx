...
function Board({ rows, approve }: { rows: RoadmapInitiative[]; approve: (id: string) => void }) {
  return <div className="grid gap-4 xl:grid-cols-4">{columns.map((column) => <section key={column.status} className="rounded-2xl bg-slate-100/70 p-3"><div className="mb-3 flex items-center justify-between px-1"><h2 className="text-sm font-semibold text-slate-800">{column.label}</h2><span className="rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-slate-500">{rows.filter((row) => row.status === column.status).length}</span></div><div className="space-y-3">{rows.filter((row) => row.status === column.status).map((row) => <InitiativeCard key={row.id} row={row} approve={approve} />)}{rows.filter((row) => row.status === column.status).length === 0 && <div className="rounded-xl border border-dashed border-slate-300 bg-white/60 p-5 text-center text-xs text-slate-400">No initiatives</div>}</div></section>)}</div>;
}
...
function Horizon({ label, value, active, muted }: { label: string; value: number; active?: boolean; muted?: boolean }) { return <div className={`rounded-xl border p-4 ${active ? "border-emerald-300 bg-emerald-50" : "border-[var(--border)] bg-white"} ${muted ? "opacity-60" : ""}`}><div className="text-xs text-slate-500">{label}</div><div className="mt-1 text-xl font-semibold text-slate-900">{value}</div><div className="mt-1 text-[11px] text-slate-400">{muted ? "Ready for future initiatives" : "initiatives"}</div></div>; }
