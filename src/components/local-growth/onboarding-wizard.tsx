...
function AccessIntegrations({ data, set }: { data: FormState; set: Setter }) {
  return <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="rounded-l-lg px-3 py-3">System</th><th className="px-3 py-3">Access status</th><th className="rounded-r-lg px-3 py-3">Operating note</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{integrationNames.map((name) => <tr key={name}><td className="px-3 py-3 font-semibold text-slate-800">{name}</td><td className="px-3 py-3"><select value={data.integrations[name]} onChange={(event) => set("integrations", { ...data.integrations, [name]: event.target.value as (typeof accessStates)[number] })} className="h-9 rounded-lg border border-[var(--border)] bg-white px-2 text-sm">{accessStates.map((state) => <option key={state}>{state}</option>)}</select></td><td className="px-3 py-3 text-slate-500">{data.integrations[name] === "blocked" ? "Create an access client request and dependency automatically." : data.integrations[name] === "connected" ? "Eligible for freshness/sync monitoring." : "Demo data remains available until connected."}</td></tr>)}</tbody></table></div>;
}
...
const inputClass = "h-10 w-full rounded-lg border border-[var(--border)] bg-white px-3 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100";
...
function TextArea({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string }) { return <label className="block"><Label>{label}</Label><textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} rows={3} className="mt-1.5 w-full rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100" /></label>; }
