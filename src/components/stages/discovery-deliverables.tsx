  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent><p className="text-[15px] leading-relaxed text-slate-600">{summary}</p></CardContent>
      </Card>
  ...
  return (
    <Card className="p-5">
      ...
      <ul className="flex flex-wrap gap-1.5">
        {items.map((i) => (
          <li key={i} className="rounded-md bg-[var(--surface-2)] px-2 py-1 text-xs font-medium text-slate-600 ring-1 ring-inset ring-[var(--border)]">{i}</li>
        ))}
      </ul>
    </Card>
  );
