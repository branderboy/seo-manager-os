        ...
        {funnel.map((f, i) => (
          <Card key={f.tier} className="p-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-slate-600">{f.share}% of demand</span>
            </div>
            <h3 className="mt-3 text-lg font-bold text-slate-900">{f.name}</h3>
            <p className="text-sm text-slate-600">{f.intent}</p>
            <div className="mt-4">
              <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-500">Example queries</div>
              <ul className="space-y-1.5">
                {f.queries.map((q) => (
                  <li key={q} className="text-sm text-slate-700">&ldquo;{q}&rdquo;</li>
                ))}
              </ul>
            </div>
            <div className="mt-5 flex items-center justify-between border-t border-[var(--border)] pt-4">
              <span className="text-xs font-medium uppercase tracking-wide text-slate-500">Aligned goal</span>
